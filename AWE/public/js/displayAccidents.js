...
export function displayAccidents(accidents) {
    ...
    let columns = Object.keys(firstAccident).filter((col) => !flagColumns.includes(col.toLowerCase()));
    const hasFlags = flagColumns.some((flag) => Object.keys(firstAccident).some((col) => col.toLowerCase() === flag));
    if (hasFlags)
        columns.push("flags");
