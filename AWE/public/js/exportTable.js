...
export async function exportChartToWebP(chartId, filename = "chart.webp") {
    try {
...
export async function exportChartToSVG(chartId, filename = "chart.svg") {
    try {
        const chart = Chart.getChart(chartId);
        if (!chart) {
            alert("No chart found to export.");
        ...
        // Get the chart's SVG representation
...
// Function to set loading state on export buttons
export function setExportButtonLoading(buttonId, isLoading) {
...
// Function to initialize export buttons with original text
