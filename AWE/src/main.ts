...
function populateDateTimeDropdowns() {
  ...
  (document.getElementById('start_day') as HTMLSelectElement).value = pad(now.getDate());
  (document.getElementById('start_month') as HTMLSelectElement).value = pad(now.getMonth() + 1);
  (document.getElementById('start_year') as HTMLSelectElement).value = now.getFullYear().toString();
  (document.getElementById('end_day') as HTMLSelectElement).value = pad(now.getDate());
  (document.getElementById('end_month') as HTMLSelectElement).value = pad(now.getMonth() + 1);
  (document.getElementById('end_year') as HTMLSelectElement).value = now.getFullYear().toString();
...
function updateSelectedStatesDisplay() {
  ...
  const selectedStatesDiv = document.getElementById('selected-states') as HTMLDivElement;
  
  if (selectedStates.size > 0) {
    statesList.innerHTML = '';
    selectedStates.forEach(stateCode => {
      const stateOption = document.querySelector(`#state option[value="${stateCode}"]`) as HTMLOptionElement;
...
function handleStateChange() {
  ...
  const cityInputSection = document.getElementById('city-input-section') as HTMLDivElement;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const severitySlider = document.getElementById("severity") as HTMLInputElement;
